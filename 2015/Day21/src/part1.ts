...
async function main() {
    ...
    for(const weapon of weapons) {
        for(const arm of armour) {
            for(const ring of pairedRings) {
                ...
                if(fight(you, boss)) {
                    if(currentCost < minGoldToWin) minGoldToWin = currentCost;
                    console.log(`WINNER at ${currentCost}  is ${weapon.name}, ${arm.name}, ${ring[0].name}, ${ring[1].name}`);
                }
    ...
    console.log(`Part 1 min gold to win: ${minGoldToWin}`);
}
