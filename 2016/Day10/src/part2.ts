...
async function main() {
    ...
    if (lines) {
        ...
        while(values.filter(x => x.location.startsWith('bot')).length > 0) {
            for(const b of bots) {
                if(b.valuesAtBot(values) == 2) b.process(values);
            }

            const out0 = values.filter(x => x.location === 'output 0');
            const out1 = values.filter(x => x.location === 'output 1');
            const out2 = values.filter(x => x.location === 'output 2');
            if(out0.length === 1 && out1.length === 1 && out2.length === 1) {
                const actualOut0 = (GetNumbers(out0[0].name) ?? [0])[0];
                const actualOut1 = (GetNumbers(out1[0].name) ?? [0])[0];
                const actualOut2 = (GetNumbers(out2[0].name) ?? [0])[0];
                console.log(`Part 2 product of o0,o1,o2: ${actualOut0 * actualOut1 * actualOut2}`);
                break;
            }
        }
    }
