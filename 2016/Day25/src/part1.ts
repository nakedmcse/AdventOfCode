...
class vm {
    ...
    private outAction = (arg1: string, arg2: string) : void => {
        if (this.validRegisters.includes(arg1)) {
            this.stdout.push(this.registers[this.validRegisters.indexOf(arg1)]);
        }
        else {
            const num = parseInt(arg1, 10);
    ...
    private registerActions() {
        ...
        this.Actions.set('out', this.outAction);
    }
...
async function main() {
    ...
    if (lines) {
        let a = 0;
        while (true) {
            const virt = new vm (lines, 0, [a, 0, 0, 0]);
            let possible = true;
            let checkfor = 0;
            let runlength = 0;
            while(possible && runlength < 100) {
                virt.ExecuteNext();
                if(virt.stdout.length === 0) continue;
                runlength++;
                possible = virt.stdout.pop() === checkfor;
                checkfor = checkfor === 0 ? 1 : 0;
            }
            if (possible && runlength === 100) {
                break;
            }
            a++;
        }

        console.log(`Part 1 A reg value: ${a}`);
    }
}
