...
async function main() {
    ...
    if (lines) {
        ...
        for(let i = 1024; i<corrupted.length; i++) {
            const stepMemMap = JSON.parse(JSON.stringify(memMap));  // copy map
            applyCorruption(stepMemMap, corrupted, i);
            if(minStepsToTarget(stepMemMap,[0,0],[70,70]) === -1) {
                // print first location to cause complete blockage
                console.log(`Part 2: ${corrupted[i-1][0]}, ${corrupted[i-1][1]}`);
                break;
            }
        }
    }
